...
public class ExonTxtStrategy {
    ...
    public static void define(final StrategyStruct struct) {
        final var strategyPoint = struct.getStrategyPoint();
        struct.setStrategyPoint(ERROR);
        final var ch = struct.getCh();

        switch (strategyPoint) {
            case OBJ_L_CURLY_BRACKET -> {
                if (ch == RIGHT_CURLY_BRACKET) {
                    struct.setStrategyPoint(CharPlaceType.OBJ_R_CURLY_BRACKET);
                } else if (Character.isWhitespace(ch)) {
                    struct.setStrategyPoint(OBJ_WHITE_SPACE_1);
                } else if (Character.isLetter(ch)) {
                    struct.setStrategyPoint(OBJ_MEMBER_CHARACTER_FIRST);
                }
            }
            ...
            case OBJ_WHITE_SPACE_1 -> {
                if (Character.isWhitespace(ch)) {
                    ...
                    struct.setStrategyPoint(OBJ_R_CURLY_BRACKET);
                } else if (Character.isLetter(ch)) {
                    struct.setStrategyPoint(OBJ_MEMBER_CHARACTER_FIRST);
                }
            }
            case OBJ_MEMBER_CHARACTER_FIRST, OBJ_MEMBER_CHARACTER_AFTER_FIRST -> {
                if (Character.isLetter(ch)) {
                    struct.setStrategyPoint(OBJ_MEMBER_CHARACTER_AFTER_FIRST);
                } else if (Character.isWhitespace(ch)) {
                    struct.setStrategyPoint(OBJ_WHITE_SPACE_2);
                } else if (ch == COLON) {
                    struct.setStrategyPoint(OBJ_COLON);
                }
            }
            ...
            case OBJ_COLON, OBJ_WHITE_SPACE_3 -> {
                if (Character.isWhitespace(ch)) {
                    struct.setStrategyPoint(OBJ_WHITE_SPACE_3);
                } else {
                    struct.setStrategyPoint(defineValueIntoObjAndArr(ch));
                }
            }
            case ARR_L_SQUARE_BRACKET, ARR_WHITE_SPACE -> {
                if (ch == RIGHT_SQUARE_BRACKET) {
                    struct.setStrategyPoint(ARR_R_SQUARE_BRACKET);
                } else if (Character.isWhitespace(ch)) {
                    struct.setStrategyPoint(ARR_WHITE_SPACE);
                } else {
                    struct.setStrategyPoint(defineValueIntoObjAndArr(ch));
                }
            }
            case STR_L_APOSTROPHE -> {
                if (ch == APOSTROPHE) {
                    struct.setStrategyPoint(STR_R_APOSTROPHE);
                } else if (Character.isLetter(ch)) {
                    struct.setStrategyPoint(STR_CHARACTER);
                }
            }
            case STR_CHARACTER -> {
                if (Character.isLetter(ch)) {
            ...
            case NUM_DIGIT_FIRST, NUM_DIGIT_AFTER_FIRST -> {
                if (Character.isDigit(ch)) {
                    struct.setStrategyPoint(NUM_DIGIT_AFTER_FIRST);
                } else if (Character.isWhitespace(ch)) {
                    struct.setStrategyPoint(NUM_WHITE_SPACE);
                }
            }
            case DATE_L_QUOTATION_MARKS -> {
                //todo check format
                if (Character.isDigit(ch) || dateCharacters.contains(ch)) {
                    struct.setStrategyPoint(DATE_CHARACTER);
                }
            }
            case DATE_CHARACTER -> {
                if (Character.isDigit(ch) || dateCharacters.contains(ch)) {
                    ...
                    struct.setStrategyPoint(DATE_R_QUOTATION_MARKS);
                }
            }
            case PATH_L_GRAVE_ACCENT -> {
                if (Character.isLetter(ch) || pathCharacters.contains(ch)) {
                    struct.setStrategyPoint(PATH_CHARACTER);
                }
            }
            case PATH_CHARACTER -> {
                if (Character.isLetter(ch) || pathCharacters.contains(ch)) {
                    ...
                    struct.setStrategyPoint(PATH_R_GRAVE_ACCENT);
                }
            }
            case BOOL_TRUE_T -> {
                if (ch == bool_true_r) {
                    struct.setStrategyPoint(BOOL_TRUE_R);
                }
            }
            case BOOL_TRUE_R -> {
                if (ch == bool_true_u) {
                    struct.setStrategyPoint(BOOL_TRUE_U);
                }
            }
            case BOOL_TRUE_U -> {
                if (ch == bool_true_e) {
                    struct.setStrategyPoint(BOOL_TRUE_E);
                }
            }
            case BOOL_TRUE_E, BOOL_FALSE_E, BOOL_NIL_L -> {
                if (Character.isWhitespace(ch)) {
                    struct.setStrategyPoint(BOOL_WHITE_SPACE);
                }
            }
            case BOOL_FALSE_F -> {
                if (ch == bool_false_a) {
                    struct.setStrategyPoint(BOOL_FALSE_A);
                }
            }
            case BOOL_FALSE_A -> {
                if (ch == bool_false_l) {
                    struct.setStrategyPoint(BOOL_FALSE_L);
                }
            }
            case BOOL_FALSE_L -> {
                if (ch == bool_false_s) {
                    struct.setStrategyPoint(BOOL_FALSE_S);
                }
            }
            case BOOL_FALSE_S -> {
                if (ch == bool_false_e) {
                    struct.setStrategyPoint(BOOL_FALSE_E);
                }
            }
            case BOOL_NIL_N -> {
                if (ch == bool_nil_i) {
                    struct.setStrategyPoint(BOOL_NIL_I);
                }
            }
            case BOOL_NIL_I -> {
                if (ch == bool_nil_l) {
                    struct.setStrategyPoint(BOOL_NIL_L);
                }
            }
        }
    }
